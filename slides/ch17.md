...
- ブログには、Draft, PendingReview, Approvedの3状態がある
- レビューを要求すると、DraftからPendingReviewに遷移する
- 承認されると PendingReview からApprovedに遷移する
...
- ステートパターンの短所
  ...
  - ロジックの重複が多い
    - 各Stateの`request_review()`と`approved()`
    - Postの`request_review()`と`approved()`がほぼ同じ
    - (私見) Publishedに `approved()`が実装されるなど、ありえない状態遷移も実装される
